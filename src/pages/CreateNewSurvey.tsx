...
export default function CreateNewSurvey() {
  ...
  // Reads the uploaded image as a Base64 data URL and stores it
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setFormData((prev) => ({
          ...prev,
          logo: reader.result as string, // "data:image/png;base64,..."
        }));
      };
    ...
    <div className="flex min-h-screen flex-col items-center bg-[#F8FAFC]">
      <div className="flex max-w-200 py-10 px-6 flex-col gap-8 w-full">
        ...
        <div className="flex justify-end w-full">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 text-[#64748B] text-sm font-medium hover:text-[#1E293B]"
          >
            <svg
              width="16"
              height="16"
        ...
        <div className="flex flex-col items-center gap-2 text-center mb-4">
          ...
          <p className="text-[#64748B] text-sm">
            Create from scratch or use AI to generate one instantly.
          </p>
        </div>

        {/* Mode Toggle */}
        <div className="flex bg-gray-100 rounded-xl p-1 w-fit mx-auto border border-[#E2E8F0]">
          <button
            ...
            <>
              ...
              <section className="flex flex-col gap-6">
                <div className="flex justify-between items-center border-b border-[#F1F5F9] pb-4">
                  ...
                  >
                    <div
                      className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-200 ${formData.customizeBranding ? "left-6" : "left-1"}`}
                    ></div>
                  </div>
                </div>
                ...
                {formData.customizeBranding && (
                  <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-top-2 duration-300">
                    <label className="flex flex-col items-center justify-center border-2 border-dashed border-[#CBD5E1] rounded-lg p-6 bg-[#F8FAFC] gap-3 cursor-pointer hover:bg-slate-50 transition-colors">
                      <input
                        type="file"
                        className="hidden"
                        accept="image/*"
                      ...
                      <span className="mt-1 px-6 py-2 bg-[#E2E8F0] text-[#1E293B] text-xs font-bold rounded-md">
                    ...
                    <div className="grid grid-cols-1 gap-6 text-left">
                      <div className="flex flex-col gap-2">
                        ...
                        <input
                          ...
                          className="w-full p-3 border border-[#E2E8F0] rounded-lg bg-[#F8FAFC] text-sm outline-none focus:ring-1 focus:ring-blue-400"
                        />
                      </div>
              ...
              <section className="flex flex-col gap-6 text-left">
                <h2 className="text-[#1E293B] text-sm font-bold uppercase tracking-wider border-b border-[#F1F5F9] pb-4">
                  Survey Details
                </h2>
                ...
                <button
                  onClick={() => {
                    const finalData = {
                      ...formData,
                      surveyTitle:
                        formData.surveyTitle.trim() || "Untitled Survey",
                    };
                    navigate("/add-questions", {
                      state: { formData: finalData },
                    });
                  }}
