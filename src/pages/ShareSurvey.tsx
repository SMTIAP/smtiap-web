...
export default function ShareSurvey() {
  ...
  const handleToggle = async () => {
    ...
    setIsPasswordProtected(newValue); setPasswordSaved(false); setPassword('');
  ...
  const handleSavePassword = async () => {
    ...
    } catch (err) { console.error("Failed to save password settings:", err); }
    ...
    <div className="flex min-h-screen flex-col items-center bg-[#F8FAFC] dark:bg-[#0F172A] py-12 px-6 font-sans transition-colors duration-300">
      <div className="max-w-xl w-full bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-200 dark:border-slate-700 p-10 text-center transition-colors duration-300">
        ...
        <div className="flex justify-start mb-6">
          <button onClick={() => navigate('/created-surveys')}
        ...
        <h1 className="text-2xl font-black text-slate-900 dark:text-white mb-2">Survey Published!</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">Share the link or download the QR code to start collecting responses.</p>
        ...
        <div className="mb-6 text-left bg-slate-50 dark:bg-slate-900 rounded-2xl p-5 border border-slate-100 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-slate-700 dark:text-slate-300">Password Protection</span>
            </div>
            <div
          ...
          {isPasswordProtected && (
            <div className="flex gap-2 mt-4">
              <input
                type="password" placeholder="Set a password" value={password}
        ...
        <div className="flex flex-col items-center bg-slate-50 dark:bg-slate-900 rounded-3xl p-8 border border-slate-100 dark:border-slate-700">
          ...
          <button onClick={downloadQR} className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-bold text-sm hover:text-indigo-800 transition-colors">
