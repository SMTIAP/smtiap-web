...
const DeleteConfirmModal = ({ ...props }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-md transition-all duration-300" onClick={props.onCancel} />
      <div className="relative bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 dark:border-slate-700/50 p-6 max-w-xs w-full z-10 animate-in fade-in zoom-in duration-200">
        <button onClick={props.onCancel} className="absolute top-3 right-3 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 transition-all">
          <X size={14} />
        </button>
        <div className="flex items-center gap-3 mb-4">
          ...
          <p className="font-bold text-slate-900 dark:text-white text-base">Delete survey?</p>
        </div>
        ...
        <div className="flex gap-2">
          <button onClick={props.onCancel} disabled={props.deleting} className="flex-1 py-2.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-semibold text-sm hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">
            Cancel
          </button>
          <button onClick={props.onConfirm} disabled={props.deleting} className="flex-1 py-2.5 bg-gradient-to-r from-rose-500 to-rose-600 text-white rounded-xl font-semibold text-sm hover:from-rose-600 hover:to-rose-700 transition-all disabled:opacity-50 shadow-md">
...
const ShareModal = ({ survey, onClose }: { survey: SurveyItem; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 dark:border-slate-700/50 p-5 max-w-sm w-full z-10 max-h-[85vh] overflow-y-auto animate-in fade-in zoom-in duration-200">
        <button onClick={onClose} className="absolute top-3 right-3 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 transition-all">
          <X size={14} />
        </button>
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <div>
            <p className="font-bold text-slate-900 dark:text-white text-sm">Share Survey</p>
        ...
        <div className="mb-4 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 rounded-xl p-3 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-semibold text-slate-700 dark:text-slate-300">Password Protection</span>
            </div>
            <div onClick={handleToggle} className={`w-8 h-4 rounded-full relative cursor-pointer transition-all duration-200 ${isPasswordProtected ? "bg-indigo-600" : "bg-slate-300 dark:bg-slate-600"}`}>
              <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all duration-200 shadow-sm ${isPasswordProtected ? "left-4" : "left-0.5"}`} />
            </div>
          </div>
          {isPasswordProtected && (
            <div className="flex gap-2 mt-2.5">
              <input type="password" placeholder={survey.isPasswordProtected ? "••••••••" : "Set a password"} value={password} onChange={(e) => { setPassword(e.target.value); setPasswordSaved(false); }} className="flex-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-1.5 text-xs outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all text-slate-800 dark:text-white" />
              <button onClick={handleSavePassword} disabled={!password || savingPassword} className="px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-lg text-xs font-semibold hover:from-indigo-700 hover:to-indigo-800 disabled:opacity-40 transition-all min-w-[50px] shadow-sm">
                {passwordSaved ? "✓" : savingPassword ? "..." : "Save"}
              </button>
            </div>
          )}
          {!isPasswordProtected && <p className="text-[10px] text-slate-400 mt-1">Anyone with the link can access this survey.</p>}
        </div>

        <p className="text-[10px] font-semibold uppercase text-slate-400 tracking-wider mb-1.5">Survey Link</p>
        <div className="flex gap-2 mb-4">
          <input readOnly value={surveyLink} className="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-[11px] font-medium text-slate-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-indigo-100" />
          <button onClick={copyToClipboard} className="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white px-3 rounded-xl hover:from-indigo-700 hover:to-indigo-800 transition-all flex items-center justify-center min-w-[38px] shadow-sm">
            {copied ? <Check size={13} /> : <Copy size={13} />}
          </button>
        </div>

        <p className="text-[10px] font-semibold uppercase text-slate-400 tracking-wider mb-2">QR Code</p>
        <div className="flex items-center gap-4 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 rounded-xl p-3 border border-slate-200 dark:border-slate-700">
          <div className="bg-white p-2 rounded-xl shadow-md shrink-0">
            <QRCodeSVG id="share-modal-qr" value={surveyLink} size={90} level="H" includeMargin={true} />
          </div>
          <div className="flex flex-col gap-2">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-snug">Download QR code to share</p>
            <button onClick={downloadQR} className="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400 font-semibold text-xs hover:text-indigo-800 transition-colors">
...
export default function CreatedSurveys() {
  ...
  const canCreate = effectiveRole ? ["super_admin", "admin", "creator"].includes(effectiveRole) : true;
  ...
  const fetchSurveys = async () => {
    ...
    try {
      ...
      if (activeTenantId && activeTenantId !== "__system__") headers["x-tenant-id"] = activeTenantId;
  ...
  useEffect(() => { fetchSurveys(); }, [activeTenant]);
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === "surveyUpdated") { fetchSurveys(); localStorage.removeItem("surveyUpdated"); }
    };
  ...
  const handleCardClick = (survey: SurveyItem) => {
    if (survey.status === "Draft") {
      if (canCreate) navigate("/add-questions", { state: { surveyId: survey._id } });
      else navigate("/add-questions", { state: { surveyId: survey._id, readOnly: true } });
  ...
  const handleDeleteClick = (event: React.MouseEvent, survey: SurveyItem) => { event.stopPropagation(); setSurveyToDelete(survey); setDeleteError(null); setShowDeleteModal(true); };

  const token = localStorage.getItem("token");
  const authHeaders = (): Record<string, string> => ({
    "Content-Type": "application/json",
    ...(token ? { Authorization: `Bearer ${token}` } : {}),
    ...(() => { const id = localStorage.getItem("activeTenantId"); return id && id !== "__system__" ? { "x-tenant-id": id } : {}; })(),
  });
  ...
  const handleCopyClick = async (e: React.MouseEvent, survey: SurveyItem) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/surveys/${survey._id}`, { headers: token ? { Authorization: `Bearer ${token}` } : {}, credentials: "include" });
    ...
    } catch (err) { console.error("Failed to copy survey:", err); } finally { setCopyingSurveyId(null); }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const activeTenantId = localStorage.getItem("activeTenantId");
      const response = await fetch(`http://localhost:5000/api/surveys/${surveyToDelete._id}`, {
        method: "DELETE",
        headers: { ...(currentToken ? { Authorization: `Bearer ${currentToken}` } : {}), ...(activeTenantId && activeTenantId !== "__system__" ? { "x-tenant-id": activeTenantId } : {}) },
        credentials: "include",
      });
      if (!response.ok) {
        let msg = "Failed to delete. Please try again.";
        try { const errData = await response.json(); msg = errData.message || msg; } catch { }
        setDeleteError(msg);
        return;
      }
      await fetchSurveys();
    ...
    } catch (err) { console.error("Failed to delete survey:", err); setDeleteError("Network error. Is the server running?"); } finally { setDeletingSurveyId(null); }
  };

  const filteredSurveys = surveys.filter((survey) => activeTab === "All" || survey.status === activeTab);
  ...
  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-indigo-600 dark:text-indigo-400 font-semibold tracking-wide uppercase text-xs animate-pulse">Loading Workspace</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      {showDeleteModal && surveyToDelete && <DeleteConfirmModal survey={surveyToDelete} onConfirm={handleConfirmDelete} onCancel={() => { setShowDeleteModal(false); setSurveyToDelete(null); setDeleteError(null); }} deleting={deletingSurveyId === surveyToDelete._id} errorMsg={deleteError} />}
      {surveyToShare && <ShareModal survey={surveyToShare} onClose={() => setSurveyToShare(null)} />}

      <div className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 animate-gradient-x z-50" />
          ...
          {filteredSurveys.map((survey) => {
            ...
            return (
              ...
              >
                ...
                <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${getStatusColor()}`} />
                ...
                <div className="flex flex-col items-center justify-center flex-grow text-center w-full">
                  ...
                  {isScheduled && survey.scheduledOpen && (
                    <div className="flex items-center gap-1 text-purple-600 text-[10px] font-bold">
                      <Calendar size={10} />
                      <span>{new Date(survey.scheduledOpen).toLocaleDateString()}</span>
                    </div>
                  )}
                </div>
