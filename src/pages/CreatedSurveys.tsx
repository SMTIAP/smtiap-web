...
const DeleteConfirmModal = ({ survey, onConfirm, onCancel, deleting }: {
  survey: SurveyItem; onConfirm: () => void; onCancel: () => void; deleting: boolean;
}) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
    ...
    <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700 p-6 max-w-xs w-full z-10">
      <button onClick={onCancel} className="absolute top-3 right-3 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 transition-all">
        <X size={14} />
      </button>
      <div className="flex items-center gap-3 mb-4">
        <div className="w-9 h-9 bg-rose-50 dark:bg-rose-900/40 rounded-xl flex items-center justify-center shrink-0">
          <Trash2 size={16} className="text-rose-500" />
        </div>
        <p className="font-black text-slate-900 dark:text-white text-base">Delete survey?</p>
      </div>
      <p className="text-slate-700 dark:text-slate-300 text-sm font-semibold truncate mb-1">"{survey.surveyTitle || "Untitled Survey"}"</p>
      <p className="text-slate-400 text-xs mb-6">This can't be undone.</p>
      <div className="flex gap-2">
        <button onClick={onCancel} disabled={deleting} className="flex-1 py-2.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold text-sm hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">Cancel</button>
...
const ShareModal = ({ survey, onClose }: { survey: SurveyItem; onClose: () => void; }) => {
  ...
  const copyToClipboard = () => { navigator.clipboard.writeText(surveyLink); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  ...
  const handleToggle = async () => {
    ...
    setIsPasswordProtected(newValue); setPassword(""); setPasswordSaved(false);
    if (!newValue) {
      try {
      ...
      } catch (err) { console.error("Failed to clear password:", err); }
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700 p-5 max-w-sm w-full z-10 max-h-[85vh] overflow-y-auto mt-16">
        <button onClick={onClose} className="absolute top-3 right-3 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 transition-all"><X size={14} /></button>
        <div className="flex items-center gap-2.5 mb-4">
          <div className="w-8 h-8 bg-indigo-50 dark:bg-indigo-900/40 rounded-xl flex items-center justify-center shrink-0">
            <Share2 size={14} className="text-indigo-600 dark:text-indigo-400" />
          </div>
          <div>
            <p className="font-black text-slate-900 dark:text-white text-sm">Share survey</p>
            <p className="text-slate-400 text-[10px] truncate max-w-[200px]">{survey.surveyTitle || "Untitled Survey"}</p>
          </div>
        </div>

        <div className="mb-4 bg-slate-50 dark:bg-slate-900 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <Lock size={12} className="text-slate-500" />
              <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300">Password Protection</span>
            </div>
            <div onClick={handleToggle} className={`w-8 h-4 rounded-full relative cursor-pointer transition-all duration-200 ${isPasswordProtected ? "bg-indigo-600" : "bg-slate-200 dark:bg-slate-600"}`}>
          ...
          {isPasswordProtected && (
            <div className="flex gap-2 mt-2.5">
              <input type="password" placeholder={survey.isPasswordProtected ? "••••••••" : "Set a password"} value={password}
                onChange={(e) => { setPassword(e.target.value); setPasswordSaved(false); }}
                className="flex-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-1.5 text-xs outline-none focus:border-indigo-400 transition-all text-slate-800 dark:text-white" />
              <button onClick={handleSavePassword} disabled={!password || savingPassword}
                className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 disabled:opacity-40 transition-all min-w-[50px]">
                {passwordSaved ? "✓" : savingPassword ? "..." : "Save"}
              </button>
            </div>
          )}
          {!isPasswordProtected && <p className="text-[10px] text-slate-400 mt-1">Anyone with the link can access this.</p>}
        </div>

        <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest mb-1.5">Survey link</p>
        <div className="flex gap-2 mb-4">
          <input readOnly value={surveyLink} className="flex-1 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-[11px] font-medium text-slate-600 dark:text-slate-300 outline-none" />
          <button onClick={copyToClipboard} className="bg-indigo-600 text-white px-3 rounded-xl hover:bg-indigo-700 transition-all flex items-center justify-center min-w-[38px]">
            {copied ? <Check size={13} /> : <Copy size={13} />}
          </button>
        </div>

        <p className="text-[10px] font-bold uppercase text-slate-400 tracking-widest mb-2">QR code</p>
        <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-900 rounded-xl p-3 border border-slate-100 dark:border-slate-700">
          <div className="bg-white p-2 rounded-xl shadow-sm shrink-0">
            <QRCodeSVG id="share-modal-qr" value={surveyLink} size={90} level="H" includeMargin={true} />
          </div>
          <div className="flex flex-col gap-2">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-snug">Download & share survey QR</p>
            <button onClick={downloadQR} className="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400 font-bold text-xs hover:text-indigo-800 transition-colors">
...
export default function CreatedSurveys() {
  ...
  const handleCardClick = (survey: SurveyItem) => {
    if (survey.status === "Draft") navigate("/add-questions", { state: { surveyId: survey._id } });
    else if (survey.status === "Running" || survey.status === "Finished") navigate(`/survey-results/${survey._id}`);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/surveys/${surveyToDelete._id}`, { method: "DELETE" });
      if (!response.ok) throw new Error("Failed to delete");
      setSurveys((prev) => prev.filter((item) => item._id !== surveyToDelete._id));
      setShowDeleteModal(false); setSurveyToDelete(null);
    } catch (err) { console.error("Failed to delete survey:", err); }
  ...
  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-[#F8FAFC] dark:bg-[#0F172A]">
      <div className="animate-pulse flex flex-col items-center gap-4">
        <div className="h-12 w-12 rounded-full bg-indigo-200 dark:bg-indigo-800"></div>
        <p className="text-indigo-900 dark:text-indigo-300 font-bold tracking-widest uppercase text-xs">Loading Workspace</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-[#FDFDFD] dark:bg-[#0F172A] transition-colors duration-300">
      {showDeleteModal && surveyToDelete && (
        <DeleteConfirmModal survey={surveyToDelete} onConfirm={handleConfirmDelete}
      ...
      <div className="flex max-w-[1200px] py-12 px-8 flex-col items-start gap-10 w-full">
        <div className="flex justify-between items-center w-full">
          <div>
            <h1 className="text-[#0F172A] dark:text-white text-5xl font-black tracking-tight mb-2">My Surveys</h1>
            <p className="text-[#64748B] dark:text-slate-400 text-base font-medium">Track performance and draft new insights.</p>
          </div>
              ...
              <div key={survey._id} onClick={() => handleCardClick(survey)}
                ...
                <div className="flex flex-col items-center justify-center flex-grow text-center w-full">
                  <div className={`w-14 h-14 rounded-3xl flex items-center justify-center mb-6 transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 ${
                    ...
                    isDraft ? "bg-amber-50 dark:bg-amber-900/30 text-amber-500" :
