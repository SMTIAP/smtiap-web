...
import {
  Plus, GripVertical, Trash2, Settings2, Copy, Eye,
  ChevronDown, Type, List, CheckSquare, Star, Calendar,
  Hash, Layout, Save, ChevronRight, X, PlusCircle, FileText,
} from "lucide-react";

type QuestionTypeId =
  | "short_text" | "long_text" | "multiple_choice"
  | "checkboxes" | "rating" | "number" | "date";
                ...
                className={`p-2 text-sm rounded transition-colors
                  ...
                  ${selectedDate && day === selectedDate.getDate() &&
                    ...
                    selectedDate.getFullYear() === currentDate.getFullYear()
                      ...
                      : day ? "hover:bg-indigo-50 text-gray-800" : ""
                  }`}
              >
                {day}
              </button>
...
const QUESTION_TYPES: QuestionTypeConfig[] = [
  { id: "short_text", label: "Short Text", icon: <Type size={18} />, defaultData: { label: "Untitled Question", placeholder: "Enter text here..." } },
  { id: "long_text", label: "Long Text", icon: <FileText size={18} />, defaultData: { label: "Untitled Question", placeholder: "Enter detailed response..." } },
  { id: "multiple_choice", label: "Multiple Choice", icon: <List size={18} />, defaultData: { label: "Untitled Question", options: ["Option 1", "Option 2"] } },
  { id: "checkboxes", label: "Checkboxes", icon: <CheckSquare size={18} />, defaultData: { label: "Untitled Question", options: ["Option 1", "Option 2"] } },
  { id: "rating", label: "Rating", icon: <Star size={18} />, defaultData: { label: "Rate this item", max: 5 } },
  { id: "number", label: "Number", icon: <Hash size={18} />, defaultData: { label: "Enter a number", min: 0, max: 100 } },
  { id: "date", label: "Date", icon: <Calendar size={18} />, defaultData: { label: "Select a date" } },
];

// Renders a single question card in the builder canvas with move, copy and delete actions
const QuestionCard = ({
  question, index, isPreview, selectedQuestionId,
  setSelectedQuestionId, moveQuestion, duplicateQuestion,
  deleteQuestion, totalQuestions,
}: QuestionCardProps) => {
  ...
  return (
    <div
      ...
      className={`group relative bg-white rounded-xl border-2 transition-all duration-200 ${
        isActive ? "border-indigo-500 shadow-lg" : "border-gray-100 hover:border-gray-200 shadow-sm"
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        {!isPreview && isActive && (
          <div className="flex items-center gap-1">
            <button onClick={(e) => { e.stopPropagation(); moveQuestion(index, -1); }} className="p-1.5 hover:bg-gray-100 rounded-md text-gray-500" disabled={index === 0}>
              <ChevronDown className="rotate-180" size={16} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); moveQuestion(index, 1); }} className="p-1.5 hover:bg-gray-100 rounded-md text-gray-500" disabled={index === totalQuestions - 1}>
              <ChevronDown size={16} />
            </button>
            <div className="w-px h-4 bg-gray-200 mx-1" />
            <button onClick={(e) => { e.stopPropagation(); duplicateQuestion(question.id); }} className="p-1.5 hover:bg-blue-50 hover:text-blue-600 rounded-md text-gray-500">
              <Copy size={16} />
            </button>
            <button onClick={(e) => { e.stopPropagation(); deleteQuestion(question.id); }} className="p-1.5 hover:bg-red-50 hover:text-red-600 rounded-md text-gray-500">
              <Trash2 size={16} />
            </button>
          </div>
        )}
      ...
      <div className="space-y-3">
        {(question.type === "short_text" || question.type === "long_text") && (
          <input type="text" placeholder={question.placeholder} disabled className="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 text-gray-400" />
        )}
        {(question.type === "multiple_choice" || question.type === "checkboxes") && (
          <div className="space-y-2">
            {question.options?.map((opt: string, i: number) => (
              <div key={i} className="flex items-center gap-3">
                <div className={`w-4 h-4 border border-gray-300 ${question.type === "multiple_choice" ? "rounded-full" : "rounded"}`} />
        ...
        {question.type === "number" && (
          <input type="number" disabled className="bg-gray-50 border border-gray-200 rounded-lg p-2 text-gray-400 w-32" />
        )}
        {question.type === "date" && (
...
const PropertyEditor = ({
  selectedQuestion, updateQuestion, setSelectedQuestionId, branchTargets,
}: PropertyEditorProps) => {
  if (!selectedQuestion) {
    return (
  ...
  const updateBranchRule = (value: string, targetQuestionId: string) => {
    const nextRules = (branching.rules || []).filter((rule) => rule.value !== value);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setSelectedQuestionId(null)} className="p-1 hover:bg-gray-100 rounded">
          <X size={18} />
        </button>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Question Label</label>
          <input
            type="text"
                ...
                <div key={i} className="flex gap-2">
                    ...
                    onChange={(e) => {
                      ...
                      const nextBranching = selectedQuestion.branching
                        ? {
                            ...
                            rules: (selectedQuestion.branching.rules || []).map((rule) =>
                              rule.value === oldValue ? { ...rule, value: e.target.value } : rule
                            ),
                          }
                        : undefined;
                      updateQuestion(selectedQuestion.id, { options: newOpts, branching: nextBranching });
                    }}
                  ...
                  <button
                    onClick={() => {
                      ...
                      const newOpts = currentOptions.filter((_: string, idx: number) => idx !== i);
                      const nextBranching = selectedQuestion.branching
                        ? {
                            ...selectedQuestion.branching,
                            rules: (selectedQuestion.branching.rules || []).filter((rule) => rule.value !== removedOption),
                          }
                        : undefined;
                      updateQuestion(selectedQuestion.id, { options: newOpts, branching: nextBranching });
                    }}
        ...
        {supportsBranching && (
          <div className="pt-4 border-t border-gray-100 space-y-3">
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-sm font-medium text-gray-700">Enable Conditional Branching</span>
              <div
                onClick={() =>
                  updateQuestion(selectedQuestion.id, {
                    branching: branching.enabled
                      ? undefined
                      : { enabled: true, rules: branching.rules || [], defaultTargetQuestionId: branching.defaultTargetQuestionId || "" },
                  })
                }
                className={`w-11 h-6 flex items-center rounded-full p-1 transition-colors ${branching.enabled ? "bg-indigo-600" : "bg-gray-200"}`}
              >
                <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${branching.enabled ? "translate-x-5" : ""}`} />
              </div>
            </label>

            {branching.enabled && (
              <div className="space-y-3 rounded-xl border border-indigo-100 bg-indigo-50/40 p-3">
                <p className="text-xs text-gray-600">Map each answer to the next question.</p>
                {(selectedQuestion.options || []).map((optionValue) => {
                  const selectedTarget = (branching.rules || []).find((rule) => rule.value === optionValue)?.targetQuestionId;
                  return (
                    <div key={optionValue} className="grid grid-cols-2 gap-2 items-center">
                      <span className="text-xs font-semibold text-gray-700 truncate">{optionValue}</span>
                      <select
                        value={selectedTarget || ""}
                        onChange={(e) => updateBranchRule(optionValue, e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-2 items-center pt-1 border-t border-indigo-100">
                  <span className="text-xs font-semibold text-gray-700">Otherwise</span>
                  <select
                    value={branching.defaultTargetQuestionId || ""}
                    onChange={(e) =>
                      updateQuestion(selectedQuestion.id, {
                        branching: { enabled: true, rules: branching.rules || [], defaultTargetQuestionId: e.target.value },
                      })
                    }
                    className="border border-gray-300 rounded-lg p-2 text-xs bg-white"
                  >
        ...
        {selectedQuestion.type === "rating" && (
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Max Stars</label>
            <input
              type="number"
              value={selectedQuestion.max}
              onChange={(e) => updateQuestion(selectedQuestion.id, { max: parseInt(e.target.value) || 5 })}
              className="mt-1 w-full border border-gray-300 rounded-lg p-2 focus:ring-2 focus:ring-indigo-500 outline-none"
            />
          </div>
        )}
        ...
        <div className="pt-4 border-t">
          <label className="flex items-center justify-between cursor-pointer">
            ...
            >
              <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${selectedQuestion.required ? "translate-x-5" : ""}`} />
            </div>
          </label>
        </div>
      </div>
...
export default function AddQuestions() {
  ...
  const [primaryColor, setPrimaryColor] = useState(
    setupData?.customizeBranding ? setupData.themeColor : setupData?.themeColor || "#6366F1",
  );
  const [loadingSurvey, setLoadingSurvey] = useState(false);
  const [surveyTitle, setSurveyTitle] = useState(setupData?.surveyTitle || "Survey creator");
  const [pages, setPages] = useState<SurveyPage[]>([{ id: "page-1", title: "Page 1", questions: [] }]);
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/api/users/me/tenant", {
      ...
      .then((r) => r.json())
      .then((d) => { if (d.tenantId) setTenantId(d.tenantId); })
      .catch(() => {});
  }, []);
  ...
  const normalizeQuestionType = useCallback((type?: string): QuestionTypeId => {
    ...
    return allowedTypes.includes(type as QuestionTypeId) ? (type as QuestionTypeId) : "short_text";
  }, []);

  // Converts raw API page/question data into the internal SurveyPage structure
  const normalizeSurveyPages = useCallback((incomingPages: unknown): SurveyPage[] => {
    ...
    return (incomingPages as IncomingPage[]).map((page, pageIndex) => {
      ...
      return {
        ...
        questions: pageQuestions.map((question: IncomingQuestion, questionIndex: number) => {
          const type = normalizeQuestionType(question?.type);
          const supportsOptions = type === "multiple_choice" || type === "checkboxes";
          return {
            id: question?.id || question?._id || `q-${Date.now()}-${pageIndex}-${questionIndex}`,
            type,
            label: question?.label || "Untitled Question",
            placeholder: question?.placeholder ||
              (type === "short_text" ? "Enter text here..." : type === "long_text" ? "Enter detailed response..." : undefined),
              ...
              ? {
                  ...
                  rules: Array.isArray(question.branching.rules)
                    ? question.branching.rules
                        .filter((rule) => Boolean(rule?.value) && Boolean(rule?.targetQuestionId))
                        .map((rule) => ({ value: String(rule!.value), targetQuestionId: String(rule!.targetQuestionId) }))
                    : [],
                  defaultTargetQuestionId: question.branching.defaultTargetQuestionId || "",
                }
              : undefined,
          };
        }),
      ...
      const loadSurvey = async () => {
        try {
          ...
          if (res.ok) {
            ...
            setPrimaryColor(data?.primaryColor || data?.themeColor || "#6366F1");
  ...
  const addPage = () => {
    const newPage = { id: `page-${Date.now()}`, title: `Page ${pages.length + 1}`, questions: [] };
  ...
  const addQuestion = (typeId: QuestionTypeId) => {
    ...
    if (!typeDef) return;
    const newQuestion = { id: `q-${Date.now()}`, type: typeId, ...typeDef.defaultData, required: false };
  ...
  const updateQuestion = (id: string, updates: Partial<SurveyQuestion>) => {
    const updatedPages = pages.map((page) => ({
      ...page,
      questions: page.questions.map((q) => q.id === id ? { ...q, ...updates } : q),
    }));
    setPages(updatedPages);
  };
    ...
    const updatedPages = pages.map((page) => ({
      ...
      questions: page.questions
        ...
        .map((q) => {
          ...
          const nextRules = (q.branching.rules || []).filter((rule) => rule.targetQuestionId !== id);
          return {
            ...q,
            branching: {
              ...q.branching,
              rules: nextRules,
              defaultTargetQuestionId: q.branching.defaultTargetQuestionId === id ? "" : q.branching.defaultTargetQuestionId,
            },
          };
        }),
    }));
  ...
  const duplicateQuestion = (id: string) => {
    const questionToCopy = pages[activePageIndex].questions.find((q) => q.id === id);
    if (!questionToCopy) return;
    const newQuestion = { ...questionToCopy, id: `q-${Date.now()}`, branching: undefined };
    const updatedPages = [...pages];
    const index = updatedPages[activePageIndex].questions.findIndex((q) => q.id === id);
  ...
  const moveQuestion = (index: number, direction: number) => {
    ...
    if (newIndex < 0 || newIndex >= pages[activePageIndex].questions.length) return;
  ...
  const branchTargets = useMemo(() => {
    ...
    const selectedIndex = flattened.findIndex((target) => target.id === selectedQuestionId);
    ...
    <div className="flex h-screen bg-[#F8F9FB] text-gray-800 overflow-hidden font-sans">
      ...
      <aside className="w-72 bg-white border-r border-gray-200 flex flex-col z-10 shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          <div>
            <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2 mb-3">Essentials</h3>
            <div className="grid grid-cols-1 gap-2">
              {QUESTION_TYPES.map((type) => (
                ...
                >
                  <span className="p-2 bg-gray-50 group-hover:bg-white rounded-lg transition-colors shadow-sm">{type.icon}</span>
                  {type.label}
                  <PlusCircle size={14} className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest px-2 mb-3">Pages</h3>
              ...
              {pages.map((page, idx) => (
                <div
                  ...
                  className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ${
                    activePageIndex === idx ? "bg-gray-100 text-gray-900 font-semibold" : "hover:bg-gray-50 text-gray-600"
                  }`}
                >
                  <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: activePageIndex === idx ? primaryColor : "transparent" }} />
                  <span className="truncate flex-1">{page.title}</span>
                  {pages.length > 1 && (
                    <button onClick={(e) => { e.stopPropagation(); deletePage(idx); }} className="opacity-0 group-hover:opacity-100 hover:text-red-500">
                      <X size={14} />
                    </button>
                  )}
                </div>
              ))}
              <button onClick={addPage} style={{ color: primaryColor }} className="w-full flex items-center gap-2 p-2 text-sm hover:bg-gray-50 rounded-lg transition-colors mt-2">
            ...
            onClick={async () => {
              ...
              try {
                ...
                const res = await fetch(url, {
                  ...
                  body: JSON.stringify({
                    surveyTitle, primaryColor, themeColor: primaryColor,
                    pages, status: "Draft", tenantId: tenantId ?? undefined,
                  }),
                });
                const data = await res.json();
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                isPreviewMode ? "text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
              }`}
            >
              <Eye size={16} />
              ...
              <div className="space-y-4">
                ...
                {activePage.questions.length === 0 ? (
                  ...
                  activePage.questions.map((q, idx) => (
                    <QuestionCard
                      key={q.id} question={q} index={idx} isPreview={false}
              ...
              <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden min-h-150 flex flex-col">
                  ...
                  <div className="space-y-8">
                    {activePage.questions.map((q, idx) => (
                      <div key={q.id}>
                        ...
                        <div className="pl-6">
                          {q.type === "short_text" && (
                            <input type="text" className="w-full border-b-2 border-gray-100 focus:border-gray-400 outline-none pb-2 transition-colors" style={{ caretColor: primaryColor }} placeholder={q.placeholder} />
                          )}
                          {q.type === "long_text" && (
                            <textarea className="w-full border-2 border-gray-100 rounded-xl p-3 outline-none min-h-25 transition-colors focus:border-gray-400" placeholder={q.placeholder} />
                          )}
                          ...
                          {q.type === "rating" && (
                            <div className="flex gap-2">
                              {[...Array(q.max)].map((_, i) => (
                                <button key={i} className="text-gray-200 hover:text-yellow-400 transition-colors">
                                  <Star size={32} />
                                </button>
                              ))}
                            </div>
                          ...
                          {q.type === "number" && (
                            <input type="number" className="border-2 border-gray-100 rounded-lg p-2 outline-none w-32 focus:border-gray-400" />
                          )}
                          {q.type === "date" && (
                            <DatePickerCalendar
                              value={responses[q.id] || ""}
                              onChange={(date) => setResponses((prev) => ({ ...prev, [q.id]: date }))}
                            />
                          )}
                          {q.branching?.enabled && (
                ...
                <div className="p-8 bg-gray-50 flex justify-between items-center border-t border-gray-100">
                  ...
                  <div className="flex gap-4">
                    {activePageIndex > 0 && (
                      <button onClick={() => setActivePageIndex(activePageIndex - 1)} className="px-6 py-2 border border-gray-200 rounded-xl font-semibold hover:bg-white transition-all">
                        Back
                      </button>
                    )}
