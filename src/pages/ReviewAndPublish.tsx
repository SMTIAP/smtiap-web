...
// Modal for setting scheduled open/close dates with 12-hour time picker.
function ScheduleModal({
  ...
  // Validate that dates are in the future and close is after open, then save.
  const handleSave = () => {
    ...
    if (enableClose && closeDate) {
      ...
      if (enableOpen && openDate && openDateTime) {
        if (localDate <= new Date(openDateTime)) {
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 p-6 w-full max-w-md">
        ...
        <div className="border-b border-slate-100 dark:border-slate-700 pb-4 mb-4">
          <label className="flex items-center justify-between cursor-pointer mb-3">
            ...
            >
              <div
                className={`bg-white w-3.5 h-3.5 rounded-full shadow-md transform transition-transform ${enableOpen ? "translate-x-5" : ""}`}
              />
            </div>
          </label>
              ...
              <div className="flex gap-1 items-center">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setOpenHour(v.toString().padStart(2, "0"));
                  }}
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setOpenMinute(v.toString().padStart(2, "0"));
                  }}
                  className="w-20 px-2 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 text-center"
                />
                <select
...
// Survey review page before publishing, with schedule and draft/save options.
export default function ReviewAndPublish() {
  ...
  // Save the survey with chosen status, then navigate to the appropriate next screen.
      ...
      if (status === "Running" || status === "Scheduled") {
        navigate("/share-survey", {
          state: {
            ...
            scheduledOpen:
              status === "Scheduled" ? savedSurvey.scheduledOpen : null,
          },
        });
      } else {
        navigate("/created-surveys", {
  ...
  // Build human-readable schedule summary for the review section.
  const scheduleSummary = () => {
    const open = scheduledOpen
      ? new Date(scheduledOpen).toLocaleString()
      : null;
    const close = scheduledClose
      ? new Date(scheduledClose).toLocaleString()
      : null;
          ...
          <div className="p-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
              <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-700">
                ...
                <p className="text-gray-900 dark:text-white font-bold break-words whitespace-normal">
                  {surveyTitle}
                </p>
              </div>
              {description && (
                <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-700">
                  ...
                  <p className="text-gray-700 dark:text-slate-300 text-sm break-words whitespace-normal">
                    {description}
                  </p>
                </div>
              )}
              ...
              <div className="p-4 bg-gray-50 dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-700">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 border border-purple-100 dark:border-purple-800 font-bold">
                    Scheduled
                  </span>
                </div>
              </div>
            ...
            {hasSchedule && (
              <div className="mb-6 p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-200 dark:border-indigo-800">
                ...
                <p className="text-xs text-indigo-600 dark:text-indigo-400 mt-1 break-words whitespace-normal">
                  {scheduleSummary()}
                </p>
              </div>
            )}
                ...
                {pages.map((page: any, pIdx: number) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-gray-800 dark:text-white break-words whitespace-normal">
                        {page.title}
                      </span>
                    </div>
                        ...
                        {page.questions.map((q: any, qIdx: number) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold text-gray-800 dark:text-white mb-1 break-words whitespace-normal">
                                {q.label}
                              </p>
                                ...
                                <div className="mt-2 flex flex-wrap gap-1">
                                  {q.options
                                    ...
                                    .map((opt: string, i: number) => (
                                      <span
                                        key={i}
                                        className="text-[10px] px-2 py-0.5 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded text-gray-500 dark:text-slate-400 break-words"
                                      >
                                        {opt}
                                      </span>
                                    ))}
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-3 pt-8 border-t border-gray-100 dark:border-slate-700">
              ...
              <button
                onClick={() =>
                  handleFinalize(scheduledOpen ? "Scheduled" : "Running")
                }
