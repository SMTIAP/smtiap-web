...
export default function ReviewAndPublish() {
  ...
  // This looks for the data we sent from the previous pages
  const surveyTitle = location.state?.surveyData?.surveyTitle || "Untitled Survey";
  const questionText = location.state?.questionData?.text || "No question text entered";
  const options = location.state?.questionData?.options || [];

  return (
    <div className="flex min-h-screen flex-col items-center bg-[#F8FAFC]">
      <div className="flex max-w-[900px] py-10 px-6 flex-col gap-6 w-full">
        ...
        <div className="bg-white p-12 rounded-xl shadow-sm border border-[#E2E8F0]">
          ...
          <div className="flex flex-col gap-6 text-sm mb-10">
            <div className="flex justify-between items-center border-b border-[#F1F5F9] py-3">
              ...
              {/* THIS SHOWS WHAT YOU TYPED */}
              <span className="text-[#1E293B] font-bold text-base">{surveyTitle}</span>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3">
            <button className="px-6 py-2.5 bg-[#E2E8F0] text-[#1E293B] rounded-lg font-bold text-xs hover:bg-gray-200">
              Save as draft
            </button>
            <button className="px-6 py-2.5 bg-[#22C55E] text-white rounded-lg font-bold text-xs hover:bg-[#16A34A] shadow-md">
              Publish Now
            </button>
          </div>
        </div>
