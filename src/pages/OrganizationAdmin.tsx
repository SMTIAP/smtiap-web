...
import {
  Users, CreditCard, Plus, ChevronRight, FileText, CalendarDays, type LucideIcon,
} from "lucide-react";
...
const FeatureCard = ({ to, icon: Icon, gradient, title, description, badge, badgeColor }: FeatureCardProps) => {
  const content = (
    <div className="group relative bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-xl hover:scale-[1.02] hover:border-indigo-300 dark:hover:border-indigo-500 transition-all duration-300 p-6 h-full flex flex-col cursor-pointer">
      <div className="flex items-start justify-between mb-5">
        <div className={`flex items-center justify-center w-12 h-12 rounded-xl ${gradient} shadow-sm shrink-0`}>
          <Icon size={22} className="text-white" />
        </div>
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold ${badgeColor}`}>
          {badge}
        </span>
      </div>
      <h3 className="text-gray-900 dark:text-white font-bold text-lg mb-1.5">{title}</h3>
      <p className="text-gray-400 dark:text-slate-400 text-sm leading-relaxed flex-1 mb-5">{description}</p>
      <div className="border-t border-gray-50 dark:border-slate-700 pt-4 mt-auto">
        <span className="inline-flex items-center gap-1 text-sm font-semibold text-indigo-600 dark:text-indigo-400 group-hover:gap-2 transition-all">
          Explore Portal <ChevronRight size={16} className="transition-transform duration-300 group-hover:translate-x-1" />
        </span>
      </div>
    </div>
  );
  return to ? <Link to={to} className="w-full h-full no-underline">{content}</Link> : <div className="w-full h-full">{content}</div>;
};

interface DashboardUser { username?: string; }
interface DashboardStats {
  roles: { admin: number; creator: number; billing_manager: number; viewer: number; };
  surveys: { total: number; draft: number; published: number; ended: number; };
  subscription: { plan: string; startDate: string; endDate: string; remainingDays: number; progressPct: number; } | null;
}
...
export default function OrganizationAdmin() {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try { const res = await api.get("/me"); setUser(res.data); }
  ...
  const totalUsers = (stats?.roles.admin ?? 0) + (stats?.roles.creator ?? 0) + (stats?.roles.billing_manager ?? 0) + (stats?.roles.viewer ?? 0);
  const activePublished = stats?.surveys.published ?? 0;

  const features = [
    {
      title: "Surveys Management", icon: FileText, to: "/created-surveys",
      gradient: "bg-linear-to-br from-indigo-500 to-purple-600",
      description: "Craft, distribute and monitor active system feedback forms.",
      badge: `${activePublished} Active`, badgeColor: "text-indigo-600 bg-indigo-50 dark:bg-indigo-900/40 dark:text-indigo-300",
    },
    {
      title: "Employees & Team", icon: Users, to: "/role-management",
      gradient: "bg-linear-to-br from-indigo-500 to-purple-600",
      description: "Manage roles, permission privileges and organizational growth.",
      badge: `${totalUsers} Users`, badgeColor: "text-purple-600 bg-purple-50 dark:bg-purple-900/40 dark:text-purple-300",
    },
    {
      title: "Billing & Premium", icon: CreditCard, to: "/subscription",
      gradient: "bg-linear-to-br from-indigo-500 to-purple-600",
      description: "Configure licensing, view active invoices, or scale your account.",
      badge: stats?.subscription?.plan ? `${stats.subscription.plan} Active` : "Free Tier",
      badgeColor: "text-rose-600 bg-rose-50 dark:bg-rose-900/40 dark:text-rose-300",
    },
  ];
      ...
      <div className="flex max-w-6xl py-10 px-6 flex-col items-start gap-10 w-full">
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="flex-1">
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-indigo-50 dark:bg-indigo-900/40 border border-indigo-100 dark:border-indigo-700 mb-4">
                ...
                <span className="text-[10px] font-bold text-indigo-700 dark:text-indigo-300 uppercase tracking-wider">Control Center Active</span>
              </div>
              ...
              <p className="text-gray-400 dark:text-slate-400 text-sm max-w-xl">
                Monitor team collaboration, active surveys, and subscription metrics seamlessly from one place.
              </p>
            </div>
            <button
        ...
        <div className="w-full">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 w-full">
            <h2 className="text-gray-800 dark:text-white text-xl font-extrabold leading-8 mb-6">Organization Status:</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 hover:shadow-md transition-shadow w-full">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <span className="text-2xl font-extrabold text-gray-900 dark:text-white">{totalUsers}</span>
                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">System Members</p>
                  </div>
                </div>
                <div className="space-y-0">
                  {statRows1.map((item, i) => (
                    <div key={i} className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-slate-700 last:border-0">
                      ...
                      <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">{item.value}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 hover:shadow-md transition-shadow w-full">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <span className="text-2xl font-extrabold text-gray-900 dark:text-white">{stats?.surveys.total ?? "—"}</span>
                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">Total Surveys</p>
                  </div>
                </div>
                <div className="space-y-0">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 hover:shadow-md transition-shadow w-full">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wider">Active Plan</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-5">
                  {[{ label: "Start", value: stats?.subscription?.startDate }, { label: "Renewal", value: stats?.subscription?.endDate }].map((d, i) => (
                    <div key={i} className="text-center p-3 rounded-xl bg-gray-50 dark:bg-slate-800">
                      <CalendarDays size={14} className="mx-auto mb-1 text-gray-400" />
                      <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">{d.label}</p>
                      <p className="text-sm font-bold text-gray-700 dark:text-slate-300 mt-0.5">{d.value ?? "—"}</p>
                    </div>
                  ))}
                </div>
                <div className="border-t border-gray-50 dark:border-slate-700 pt-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Remaining License Period</span>
                    <span className="text-sm font-extrabold text-indigo-600 dark:text-indigo-400">
                      {stats?.subscription ? `${stats.subscription.remainingDays}d` : "—"}
                    </span>
                  </div>
                  <div className="w-full h-2.5 rounded-full bg-gray-100 dark:bg-slate-700 overflow-hidden">
                    <div
                      ...
                      style={{ width: `${stats?.subscription?.progressPct ?? 0}%` }}
                    />
                  </div>
                </div>
              </div>
