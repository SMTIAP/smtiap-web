        ...
        <div className="flex min-h-screen flex-col items-center bg-[#F8FAFC] dark:bg-[#0F172A] transition-colors duration-300">
            ...
            <div className="w-full max-w-[1152px] px-6 py-10 flex flex-col gap-10">
                ...
                <div className="w-full overflow-x-auto mt-6">
                    <table className="min-w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg transition-colors duration-300">
                        <thead className="bg-gray-100 dark:bg-slate-900">
                            <tr>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Name</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Email</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Role</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredUsers.length > 0 ? (
                                filteredUsers.map((user) => (
                                    <tr key={user._id} className="border-b border-gray-300 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">{user.username}</td>
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">{user.email}</td>
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    setSelectedRole((prev) => {
                                                        const updated = { ...prev };
                                                        if (value === user.role) { delete updated[user._id]; } else { updated[user._id] = value; }
                                                        return updated;
                                                    });
                                                }}
                                        ...
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200 flex gap-2">
                                            <button
                                                onClick={() => { const tenant = tenants.find(t => t._id === selectedTenantId); if (!tenant) return; handleAddUser(user, tenant); }}
                                                disabled={!selectedTenantId}
                                                title={!selectedTenantId ? "Select an organization first" : "Add user to organization"}
                                ...
                                <tr>
                                    <td colSpan={4} className="px-6 py-4 text-center text-gray-700 dark:text-slate-400">No Users Found</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="flex items-center w-fit">
                    <h3 className="text-[#1E293B] dark:text-white font-inter font-bold text-2xl leading-9">Organization Members</h3>
                </div>

                <div className="relative w-full max-w-md">
                    <input
                        type="text" placeholder="Search organization..."
                        value={searchOrganization} onChange={(e) => setSearchOrganization(e.target.value)}
                        className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm bg-white dark:bg-slate-800 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                    />
                    <Search size={16} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
                </div>
                    ...
                    <table className="min-w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg transition-colors duration-300">
                        <thead className="bg-gray-100 dark:bg-slate-900">
                            <tr>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Name</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Email</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Current Role</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Organization</th>
                                <th className="text-left px-6 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredOrgUsers.length > 0 ? (
                                filteredOrgUsers.map((item) => (
                                    <tr key={item._id} className="border-b border-gray-300 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">{item.userId.username}</td>
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">{item.userId.email}</td>
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">
                                            <select
                                        ...
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">{item.tenantId.name}</td>
                                        <td className="px-6 py-4 text-gray-800 dark:text-slate-200">
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={() => handleRemoveOrgUser(item)}
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-4 text-center text-gray-700 dark:text-slate-400">No Organizations Found</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
