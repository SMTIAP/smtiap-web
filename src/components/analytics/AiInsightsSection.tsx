...
// Renders one keyword chip with a count and background color.
...
// Renders the full AI Insights card: run button, loading animation, error banner, findings, and tags.
  ...
  return (
    <section className="p-6 rounded-lg border border-[#CFDBE8] bg-white shadow-md relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between gap-2 mb-6 relative">
        ...
        {/* Trigger AI analysis using the handler passed from the page. */}
        <button
          onClick={runAnalysis}
      ...
      {!isAnalyzing && (
        <>
          <div className="mb-6 relative">
            ...
            {summary ? (
              <ul className="space-y-2 text-black text-base leading-relaxed">
                {/* Split summary into sentence-like chunks for bullet-point display. */}
                {summary
                  .split(". ")
                  .filter(Boolean)
                  // Render each sentence as a bullet row.
                  .map((sentence, i) => (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-slate-300 text-xs mt-1">
                  Click <span className="font-semibold">Run AI Analysis</span>{" "}
          ...
          <div className="flex flex-wrap gap-3 relative mt-8">
            {keywords.length > 0 ? (
              // Map keyword objects into colored tags.
              keywords.map((item, i) => {
                const colors = [
                  "#C5EFB5",
                  "#B5E9EF",
              ...
              <p className="text-slate-300 text-xs">No keywords extracted.</p>
            ) : null}
          </div>
        </>
      )}
